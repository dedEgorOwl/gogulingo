...
import { LoginUser, RegistrationUser } from '../../store/action-creator/currentUser';
...
const Login: React.FC = ({ setCurrentLoginType, currentLoginType }) => {
	...
	const loginSubmit = (event: SubmitEvent) => {
		...
		dispatch(LoginUser(loginInputs.username, loginInputs.password));
	};
	...
	const registerSubmit = (event: SubmitEvent) => {
		...
		dispatch(RegistrationUser(registerInputs.username, registerInputs.email, registerInputs.password));
	};
		...
		<div className={styles.wrapper}>
			...
			<div className={styles.header}>
				<div
					...
					onClick={() => ChangeModalState('login', false)}
					style={{ backgroundImage: `url('/assets/close.svg')` }}
				></div>
				<div
				...
				>
					{currentLoginType === 'login' ? (
						<>{langPackage.login[currentLang].signup.toUpperCase()}</>
					) : (
						<>{langPackage.login[currentLang].login.toUpperCase()}</>
					)}
				</div>
			</div>
